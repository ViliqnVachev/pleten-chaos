...
//		model.addAttribute("addNewMaterial", new AddMaterialBinding());
		return ViewConstants.ADD_MATERIAL_VIEW;
	}

	@PostMapping(PathConstants.ADD_MATERIAL)
	public String addMaterial(@Valid NewMaterialBinding newMaterial, BindingResult bindingResult,
			RedirectAttributes attributes) {

		// check for errors
		if (bindingResult.hasErrors()) {
			attributes.addFlashAttribute("newMaterial", newMaterial)//
					.addFlashAttribute("org.springframework.validation.BindingResult.newMaterial", bindingResult);
